...
*Solution.* To compute the launch angle that maximizes the horizontal range, we first determine the $x$-coordinate $x_d$ of the landing point as a function of $\alpha$, i.e., we find the maximum of the function $x_d$. Since the ball hits the ground when its height is zero, in the relation 

$$
 y = -\frac{g}{2v_0^2\cos^2\alpha}\cdot x^2+\frac{\sin\alpha}{\cos\alpha}\cdot x
$$

we set $y=0$ and solve it for $x$:
